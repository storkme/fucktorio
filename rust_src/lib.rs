...
struct LaneSpec {
    ...
    /// Routing strategy: 0 = axis-aligned (bus), 1 = A* free-form (spaghetti)
    strategy: u8,
...
fn route_astar(
    ...
    // TODO: Pass grid costs to astar_inner via tile_cost_fn parameter
    // For now, use the existing A* with obstacles only
    ...
    for _iteration in 0..max_iterations {
        ...
        for &idx in &order {
            ...
            let result = match spec.strategy {
                ...
                1 => route_astar(spec, &grid, obstacles, max_extent, allow_underground, ug_max_reach),
                _ => None,
            };
...
impl PyLaneSpec {
    #[new]
    #[pyo3(signature = (id, item_id, waypoints, strategy = 0, priority = 0))]
    fn new(id: u32, item_id: u16, waypoints: Vec<(i16, i16)>, strategy: u8, priority: u8) -> Self {
        PyLaneSpec { id, item_id, waypoints, strategy, priority }
    }
}
