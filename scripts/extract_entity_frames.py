...
def get_north_layers_from_draftsman(slug: str) -> list[dict]:
    ...
    anim = (
        gs.get("animation")
        or gs.get("idle_animation")
        or gs.get("on_animation")
        or e.get("animation")
        or e.get("on_animation")
...
def extract(slug: str, tile_w: int, tile_h: int) -> Path | None:
    ...
    if override:
        ...
        body_layer = next((ly for ly in layers if not ly.get("draw_as_shadow") and ly.get("width", 0) > 0), None)
        shadow_layer = next((ly for ly in layers if ly.get("draw_as_shadow") and ly.get("width", 0) > 0), None)
    ...
    if shadow_layer:
        try:
            ...
            place_on_canvas(
                canvas, shadow, shadow_layer.get("shift", [0, 0]), origin, shadow_layer.get("scale", 0.5), SHADOW_ALPHA
            )
    ...
    try:
        ...
        place_on_canvas(canvas, body, body_layer.get("shift", [0, 0]), origin, body_layer.get("scale", 0.5), 1.0)
    except Exception as e:
