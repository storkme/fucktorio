...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // Tier2 EC — iter 2 minimal repro
    //
    // Captures the exact CrossingZone state the SAT strategy produces on
    // the second growth iteration of the cluster seeded at (3, 10) for
    // tier2_electronic_circuit. Boundaries reflect interior-detection +
    // physical-feeder overrides as currently implemented; forced_empty is
    // the 11-tile set computed from the surrounding Permanent entities.
    //
    // Surrounding world (from the user's snapshot — these become Permanent
    // entities in placed_entities, and their bbox-overlapping tiles fill
    // forced_empty):
    //
    //   Iron trunk x=1:  belt(1,8) S, splitter(1,9) S, belt(1,10..12) S
    //   Iron splitter spans (1,9)+(2,9) → both feed (1,10)/(2,10) S
    //   Iron tap row y=10: belt(2,10) E, ..gap.., belt(5,10) E
    //   Copper trunk x=3: splitter(3,8) S, belt(3,9..12) S
    //   Copper trunk x=4: belt(4,9) S, belt(4,10) S
    //   Copper tap row y=11: belt(4,11) E, belt(5,11) E
    //
    // SAT zone bbox (1,8) 5×5 covers x=1..5, y=8..12. Eleven of those
    // tiles host the Permanent entities above and become forced_empty.
    //
    // The iron-plate spec enters at (2,10) E and exits at (5,10) E — its
    // crossing zone must thread items past the copper trunks at (3,10)
    // and (4,10) somehow (underground or surface detour). Interior
    // detection moves the iron IN to (2,9) S because the splitter feeds
    // it from north; copper IN moves to (3,9) S because the trunk belt
    // at (3,9) feeds the spec entry at (3,10).
    //
    // What we want SAT to produce: a solution where iron flows from
    // (2,10) to (5,10) AND copper flows from (3,10) to (3,12) — both
    // verifiable in the placed entities by item-filtered reachability.
    // What we DON'T want: a "satisfied" model whose UG corridors physically
    // cross (iron East under copper South at (3,10) is one such case;
    // Factorio doesn't allow two UG belts crossing at one tile).
    #[test]
    fn test_tier2_ec_iter2_zone() {
        let zone = CrossingZone {
            x: 1,
            y: 8,
            width: 5,
            height: 5,
            boundaries: vec![
                // Copper trunk: interior IN at (3,9), perimeter OUT at (3,12).
                ZoneBoundary {
                    x: 3, y: 9,
                    direction: EntityDirection::South,
                    item: "copper-cable".into(),
                ...
                // Iron tap: interior IN at (2,9), perimeter OUT at (5,10).
                ZoneBoundary {
                    x: 2, y: 9,
                    direction: EntityDirection::South,
                    item: "iron-plate".into(),
            ...
            forced_empty: vec![
                (1, 8),  (1, 9),  (1, 10), (1, 11), (1, 12), // iron trunk + splitter half
                (2, 9),                                       // iron splitter half
                (3, 8),  (3, 9),  (3, 11),                    // copper trunk + splitter (3,10)/(3,12) are spec ports → exempt
                (4, 9),  (4, 10),                             // copper second trunk lane
            ],
        };

        let result = solve_crossing_zone(&zone, 6, "fast-transport-belt");

        // ---- Assertion 1: SAT satisfies ----
        let solution = result.expect(
            "tier2 iter-2 zone should be SAT-satisfiable. If this fires, the \
             encoder rejected a configuration we expected it to find."
        );

        // Diagnostic dump — useful when the later assertions fail.
        eprintln!(
            "tier2 iter-2 SAT solution: {} entities ({} vars, {} clauses, {}µs)",
            solution.entities.len(),
            solution.stats.variables,
            solution.stats.clauses,
            solution.stats.solve_time_us,
        ...
        // ---- Assertion 2: iron-plate flow reaches its exit ----
        // Iron entry boundary is interior at (2,9); the in-zone neighbor
        // is (2,10). Iron exit is perimeter at (5,10).
        assert!(
            zone_flow_reaches(&solution, "iron-plate", (2, 10), (5, 10), 6),
            "iron-plate must reach (5,10) from (2,10) in the SAT solution"
        );

        // ---- Assertion 3: copper-cable flow reaches its exit ----
        // Copper entry boundary is interior at (3,9); in-zone neighbor (3,10).
        // Copper exit is perimeter at (3,12).
        assert!(
            zone_flow_reaches(&solution, "copper-cable", (3, 10), (3, 12), 6),
            "copper-cable must reach (3,12) from (3,10) in the SAT solution"
        );

        // ---- Assertion 4: no two UG corridors cross at one tile ----
        // For every UG-in/UG-out in the solution, walk its corridor
        // and assert no other UG-in/UG-out has a corridor crossing
        // through that same tile in a different axis. Catches the
        // "iron East under copper South at (3,10)" case.
        let crossings = find_crossing_ug_corridors(&solution, 6);
        assert!(
            crossings.is_empty(),
            "found {} crossing-UG configurations: {:?}",
            crossings.len(),
            crossings,
        );
    }

    /// BFS through the SAT-placed entities filtered to a single item;
    /// follows belt direction, jumps UG pairs, traverses splitter
    /// siblings (although SAT doesn't place splitters today). Returns
    /// true iff `target` is reachable from `source`.
    fn zone_flow_reaches(
        solution: &CrossingZoneSolution,
        item: &str,
        source: (i32, i32),
        target: (i32, i32),
        ...
        let entities: Vec<&PlacedEntity> = solution
            .entities
            .iter()
            .filter(|e| e.carries.as_deref() == Some(item))
            .collect();

        let mut belt_dir: std::collections::HashMap<(i32, i32), EntityDirection> =
        ...
        // Pair UG-in with nearest in-line UG-out (same direction, item).
        let mut ug_pairs: std::collections::HashMap<(i32, i32), (i32, i32)> =
            std::collections::HashMap::new();
        let mut used: std::collections::HashSet<(i32, i32)> =
            std::collections::HashSet::new();
        for inp in &ug_inputs {
            let (dx, dy) = dir_delta(entity_dir_to_idx(inp.direction));
            let mut best: Option<(i32, i32)> = None;
            let mut best_dist = i32::MAX;
            for out in &ug_outputs {
                if out.direction != inp.direction || used.contains(&(out.x, out.y)) {
                    continue;
                }
                let rx = out.x - inp.x;
                let ry = out.y - inp.y;
                ...
                if dist > 1 && dist <= max_reach as i32 && dist < best_dist {
                    best_dist = dist;
                    best = Some((out.x, out.y));
                }
            }
            if let Some(o) = best {
                ug_pairs.insert((inp.x, inp.y), o);
                ug_pairs.insert(o, (inp.x, inp.y));
                used.insert(o);
            }
        }

        if !belt_dir.contains_key(&source) {
            return false;
        }
        if source == target {
            return true;
        }
        let mut visited: std::collections::HashSet<(i32, i32)> =
            std::collections::HashSet::new();
        ...
        while let Some((x, y)) = queue.pop_front() {
            if let Some(&d) = belt_dir.get(&(x, y)) {
                let (dx, dy) = dir_delta(entity_dir_to_idx(d));
                let nb = (x + dx, y + dy);
                if (belt_dir.contains_key(&nb) || nb == target) && visited.insert(nb) {
                    if nb == target {
                        return true;
                    }
                    queue.push_back(nb);
            ...
            if let Some(&p) = ug_pairs.get(&(x, y)) {
                if visited.insert(p) {
                    if p == target {
                        return true;
                    }
    ...
    /// Detect pairs of UG corridors whose tile sequences cross. A
    /// "corridor" is the line of tiles strictly between a UG-in and its
    /// paired UG-out (matched by direction + item, nearest in-line).
    /// Returns a list of (tile, item_a, dir_a, item_b, dir_b) for each
    /// crossing tile found.
    #[allow(clippy::type_complexity)]
    fn find_crossing_ug_corridors(
        solution: &CrossingZoneSolution,
        max_reach: u32,
    ) -> Vec<((i32, i32), String, EntityDirection, String, EntityDirection)> {
        // Group UG-in/UG-out by item and pair them up.
        let mut ug_inputs: Vec<&PlacedEntity> = Vec::new();
        let mut ug_outputs: Vec<&PlacedEntity> = Vec::new();
        for e in &solution.entities {
            match e.io_type.as_deref() {
        ...
        // For each UG-in, find its corridor's mid tiles (strictly between).
        let mut corridors: Vec<((i32, i32, EntityDirection, String), Vec<(i32, i32)>)> = Vec::new();
        let mut used: std::collections::HashSet<(i32, i32)> =
            std::collections::HashSet::new();
        for inp in &ug_inputs {
            let item = inp.carries.clone().unwrap_or_default();
            let (dx, dy) = dir_delta(entity_dir_to_idx(inp.direction));
            let mut best: Option<&PlacedEntity> = None;
            let mut best_dist = i32::MAX;
            for out in &ug_outputs {
                if out.direction != inp.direction
                    || out.carries != inp.carries
                    || used.contains(&(out.x, out.y))
                {
                    continue;
                }
                let rx = out.x - inp.x;
            ...
            if let Some(out) = best {
                let mut mid = Vec::new();
                let steps = best_dist - 1;
                for k in 1..=steps {
                    mid.push((inp.x + dx * k, inp.y + dy * k));
                }
                corridors.push(((inp.x, inp.y, inp.direction, item), mid));
                used.insert((out.x, out.y));
            }
        }

        // Find tiles where two corridors of different axes cross.
        let mut crossings = Vec::new();
        for i in 0..corridors.len() {
            for j in (i + 1)..corridors.len() {
                let ((_, _, dir_a, item_a), mids_a) = &corridors[i];
                let ((_, _, dir_b, item_b), mids_b) = &corridors[j];
                let axis_a_horiz = matches!(dir_a, EntityDirection::East | EntityDirection::West);
                let axis_b_horiz = matches!(dir_b, EntityDirection::East | EntityDirection::West);
                if axis_a_horiz == axis_b_horiz {
                    continue; // same axis can't physically cross
                }
                for &t in mids_a {
                    if mids_b.contains(&t) {
                        crossings.push((t, item_a.clone(), *dir_a, item_b.clone(), *dir_b));
                    }
                }
            }
        }
